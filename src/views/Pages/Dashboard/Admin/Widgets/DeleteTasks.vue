<template>
  <card
    ...
    <div class="row">
      ...
      <div class="col">
        <slot>
          <h5 class="card-title text-uppercase text-muted mb-1">SUPPRIMER DEVOIRS/TÂCHES</h5>
  ...
  methods: {
    deleteAllTasks () {
      swal.fire({
        ...
        if (result.value) {
          this.$store.dispatch('sysconf/deleteAllTasks')
          swal.fire({
            icon: 'success',
            title: 'Succès',
            text: 'Les devoirs/tâches ont bien été supprimés.',
            customClass: {
